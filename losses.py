...
class GanLoss(nn.Module):
    ...
    def __call__(self, x, y: bool):
        ...
        loss = 0
        target_val = 1.0 if y else 0.0
        for x_i in x:
            y_i = torch.ones_like(x_i) * target_val
            loss += self.criterion(x_i, y_i)
        return loss

class WeightedLoss(nn.Module):
    def __init__(self, criterion, weight=1.0, one_minus=False):
    ...
    def __call__(self, x, y):
        val = self.criterion(x, y)
        if self.one_minus:
            val = 1 - val
        return val * self.weight

# Ding, K., Ma, K., Wang, S. and Simoncelli, E.P., 2020. Image quality assessment: Unifying structure and texture similarity. IEEE transactions on pattern analysis and machine intelligence, 44(5), pp.2567-2581.
# https://arxiv.org/abs/2004.07728
# Copyright (c) 2020 Keyan Ding
# licenses/DISTS_LICENSE
...
class DISTSLoss(torch.nn.Module):
    def __init__(self, load_weights=True):
        ...
        if load_weights:
            weights = torch.load("pretrained_models/dists.pt")
